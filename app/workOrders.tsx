...
export default function WorkOrderDetailsScreen() {
  ...
  const LOCATION_TOLERANCE_METERS = 100; // Allow 100 meters tolerance
  
  // Refs for intervals
  const stopwatchInterval = useRef<number | null>(null);
  const locationInterval = useRef<number | null>(null);

  const locPull = async (): Promise<LocationData | null> => {
    try {
      ...
      setLocationError("Location access error");
      return null;
    }
  };

  const validateLocationForWorkOrder = async (): Promise<boolean> => {
    if (!workOrder?.location?.latitude || !workOrder?.location?.longitude) {
      // If no work order location is specified, allow the operation
      return true;
    }

    const currentLoc = await locPull();
    if (!currentLoc) {
      Alert.alert(
        'Location Required',
    ...
    if (distance > LOCATION_TOLERANCE_METERS) {
      ...
      Alert.alert(
        ...
        `You are ${distanceKm} km away from the work order location. You must be within ${LOCATION_TOLERANCE_METERS}m to start/complete this work order.\n\nWork Order Location: ${workOrder.location.latitude.toFixed(6)}, ${workOrder.location.longitude.toFixed(6)}\nYour Location: ${currentLoc.latitude.toFixed(6)}, ${currentLoc.longitude.toFixed(6)}`,
        [
          { text: 'Cancel', style: 'cancel' },
          { 
            text: 'Override', 
            ...
            onPress: () => {
              Alert.alert(
                ...
                'Are you sure you want to proceed without being at the correct location?',
                [
                  { text: 'Cancel', style: 'cancel' },
                  { text: 'Proceed Anyway', style: 'destructive', onPress: () => {} }
                ]
              );
            }
          }
  ...
  const showLocationValidationSuccess = (action: 'start' | 'complete') => {
    if (!workOrder?.location?.latitude || !workOrder?.location?.longitude) {
      return; // Don't show success message if no location validation was needed
    }
    
    Alert.alert(
      'Location Verified',
      `Your location has been verified for this work order. You may proceed to ${action} the task.`,
  ...
  const handleStartWork = async () => {
    ...
    const locationValid = await validateLocationForWorkOrder();
    if (!locationValid) {
      return; // Stop here if location validation failed
    }
    
    Alert.alert(
      'Start Work Order',
      'Location verified! Are you sure you want to start this work order?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Start',
          onPress: async () => {
            try {
              ...
              // Optionally store the start location in the work order
              if (startLocation) {
                // You can add this method to your FirestoreService if needed
                // await FirestoreService.updateWorkOrderStartLocation(workOrder.id, startLocation);
              }
  ...
  const handleCompleteWork = async () => {
    ...
    Alert.alert(
      'Complete Work Order',
      'Location verified! Are you sure you want to mark this work order as completed?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Complete',
          onPress: async () => {
            try {
              ...
              // Store the current location as the end location
              const endLocation = currentLocation;
              
              await FirestoreService.updateWorkOrderStatus(workOrder.id, 'completed');
              
              // Optionally store the end location in the work order
              if (endLocation) {
                // You can add this method to your FirestoreService if needed
                // await FirestoreService.updateWorkOrderEndLocation(workOrder.id, endLocation);
              }
  ...
  const getDistanceFromWorkOrderLocation = () => {
    if (!workOrder?.location?.latitude || !workOrder?.location?.longitude || !currentLocation) {
      return null;
    }
  ...
  const renderLocationStatus = () => {
    const distance = getDistanceFromWorkOrderLocation();
    
    if (distance === null) {
      return null;
    }
    
    const isWithinTolerance = distance <= LOCATION_TOLERANCE_METERS;
    ...
    return (
      <View style={styles.locationStatusContainer}>
        <View style={[
          ...
          { backgroundColor: isWithinTolerance ? catColors.status.completed : catColors.status.cancelled }
        ]}>
          <Ionicons 
            name={isWithinTolerance ? "checkmark-circle" : "warning"} 
            size={16} 
            color={catColors.text.light} 
          />
          <Text style={styles.locationStatusText}>
            {distanceText} from work site
          </Text>
        </View>
        {!isWithinTolerance && (
          <Text style={styles.locationWarningText}>
            Must be within {LOCATION_TOLERANCE_METERS}m to start/complete work
          </Text>
        )}
      </View>
    );
